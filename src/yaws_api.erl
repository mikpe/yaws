...
%% that appearantly does not close the socket inbetween
...
stream_chunk_deliver_blocking(YawsPid, Data) ->
    ...
    after ?STREAM_GARBAGE_TIMEOUT ->
	    error_logger:error_msg(
	      ...
	      "(1 hour). Killing ~p", [?MODULE, YawsPid]),
%%% 	    error_logger:error_msg(
%%% 	      "~p:stream_chunk_deliver_blocking/2 STREAM_GARBAGE_TIMEOUT "
%%% 	      "(1 hour)~n"
%%% 	      "Killing (unless this function is catched) process tree but "
%%% 	      "NOTE that as this is probably due to that the OTP SSL "
%%% 	      "application has not managed to close the socket "
%%% 	      "(FIN_WAIT2 resp. CLOSE_WAIT) the SSL process is not killed "
%%% 	      "(it trap exit signals) and thus we will leak one file "
%%% 	      "descriptor~n"
%%% 	      "The yaws process killed is ~p with the following info:~n~p",
%%% 	      [?MODULE, YawsPid, process_info(YawsPid)]),
